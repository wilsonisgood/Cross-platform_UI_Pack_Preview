...
// import device from '../src/plugins/device'
...
var BaiduMobStat = (function () {
    var invokeNatvieMethod = function (action, obj) {
        if (isIphone || isIpad) {  
            console.info('iOS invokeNatvieMethod');
            window.webkit.messageHandlers.bmtj.postMessage({action:action, obj: obj});
        } else if (isAndroid) {            
            console.info('Android invokeNatvieMethod');
            ...
            iFrame.setAttribute('src', 'bmtj:' + encodeURIComponent(JSON.stringify(cmd)));
